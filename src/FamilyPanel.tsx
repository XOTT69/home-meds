...
import { AlertCircle, Edit3, LoaderCircle, Plus, Trash2, UsersRound, X } from 'lucide-react';
...
export function FamilyPanel({ userId, onMembersChange, readOnly = false }: { userId: string; onMembersChange?: (members: Member[]) => void; readOnly?: boolean }) {
  ...
  const close = (force = false) => {
    ...
    const original = editingId ? members.find((member) => member.id === editingId) : null;
    const originalDraft = original
      ? { name: original.name, relation: original.relation, allergies: original.allergies, notes: original.notes }
      : emptyDraft();
    if (!force && JSON.stringify(draft) !== JSON.stringify(originalDraft) && !window.confirm('Відкинути незбережені зміни?')) return;
    setOpen(false);
    setEditingId(null);
    setDraft(emptyDraft());
    setError('');
