...
function HomeMedsApp({ user }: { user: User }) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase!
        ...
        .eq('user_id', user.id);
      if (error) {
        setSyncError('Не вдалося завантажити дані. Перевірте підключення.');
        return;
      }
      setMedicines((data ?? []).filter((row) => row.kind === 'medicine').map((row) => row.payload as unknown as Medicine));
      setTravelItems((data ?? []).filter((row) => row.kind === 'travel').map((row) => row.payload as unknown as TravelItem));
    };
    void load();
  }, [user.id]);

  const saveItem = async (kind: 'medicine' | 'travel', item: Medicine | TravelItem) => {
    ...
    if (error) setSyncError('Зміни збережені на цьому пристрої, але поки не синхронізовані.');
  };

  const addMedicine = (medicine: Medicine) => {
    setMedicines((items) => [medicine, ...items]);
    void saveItem('medicine', medicine);
  };

  const addTravelItem = (item: TravelItem) => {
    setTravelItems((items) => [...items, item]);
    void saveItem('travel', item);
  };
  ...
  const toggleTravel = (id: string, key: 'packed' | 'bought') => setTravelItems((items) => items.map((item) => {
    if (item.id !== id) return item;
    const updated = { ...item, [key]: !item[key] };
    void saveItem('travel', updated);
    return updated;
  }));
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      <header className="topbar">
        ...
        <button className="profile" title="Вийти з акаунта" onClick={() => void supabase!.auth.signOut()}>{user.email?.slice(0, 1).toUpperCase() ?? 'Я'}</button>
      </header>
      {syncError && <div className="sync-error">{syncError}<button onClick={() => setSyncError('')}><X size={14} /></button></div>}
    ...
    {medicineModalOpen && <AddMedicineModal onClose={() => setMedicineModalOpen(false)} onSave={addMedicine} />}
    {travelModalOpen && <AddTravelModal onClose={() => setTravelModalOpen(false)} onSave={addTravelItem} />}
  </div>;
}
...
function TodayPage({ expiring, medicines, travelItems, onAddMedicine, onGoTravel }: { expiring: Medicine[]; medicines: Medicine[]; travelItems: TravelItem[]; onAddMedicine: () => void; onGoTravel: () => void }) {
  ...
  const attentionText = expiring.length ? `У ${expiring.length} ${expiring.length === 1 ? 'препарату' : 'препаратів'} закінчується термін придатності найближчим часом.` : 'Усі додані препарати мають актуальний строк придатності.';
  return <div className="page fade-in">
    ...
    <section className={`status-banner ${expiring.length ? '' : 'all-good'}`}><div className="banner-icon">{expiring.length ? <AlertTriangle size={22} /> : <Check size={22} />}</div><div><strong>{expiring.length ? 'Є речі, які варто перевірити' : 'Усе під контролем'}</strong><p>{attentionText}</p></div>{expiring.length > 0 && <button onClick={() => document.getElementById('expiry-section')?.scrollIntoView({ behavior: 'smooth' })}>Переглянути <ChevronRight size={17} /></button>}</section>
...
function SetupScreen() {
  return <main className="auth-shell"><section className="auth-aside"><div className="auth-brand"><span className="auth-brand-icon">+</span>home <b>meds</b></div><div className="auth-copy"><p className="eyebrow">ПІДКЛЮЧЕННЯ</p><h1>Потрібне безпечне сховище.</h1><p>Додайте параметри Supabase, щоб увімкнути авторизацію та синхронізацію.</p></div></section><section className="auth-card-wrap"><div className="auth-card"><div className="auth-icon"><ShieldCheck size={22} /></div><h2>Застосунок налаштовано</h2><p className="auth-subtitle">Залишився один безпечний крок: додати публічну URL-адресу та publishable key вашого Supabase-проєкту до змінних середовища.</p><code>VITE_SUPABASE_URL<br />VITE_SUPABASE_PUBLISHABLE_KEY</code><p className="auth-privacy">Схему бази вже додано в <code>supabase/schema.sql</code>. Ніколи не додавайте service role key у браузер.</p></div></section></main>;
}
...
export default function App() {
  ...
  if (loading) return <main className="loading-screen"><span className="auth-brand-icon">+</span><p>Відкриваємо аптечку…</p></main>;
  if (recovery && user) return <AuthScreen recovery onRecoveryComplete={() => setRecovery(false)} />;
  if (!user) return <AuthScreen />;
  return <HomeMedsApp user={user} />;
}
